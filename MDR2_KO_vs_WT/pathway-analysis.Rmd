...
subtitle: "MDR2 KO vs TGR5 transgenic Mice liver cells"
...
# Pathway analysis
The pathway analysis results are stored under [results/pathway_analysis](./results/pathway_analysis), and explained in this section.
...
```{r, fig.width=8, fig.height=20, warning=FALSE, message=FALSE, echo=FALSE}
  contrast_files <- list.files(path=paste0("results/DE_genes/"), pattern = "^(DE).*\\.(tsv)$")
  organism = "mmusculus"
  ...
  for (file in contrast_files){
    ...
    DE_genes <- read.csv(file = paste0("results/DE_genes/", file), sep="\t", header = T)
    if(nrow(DE_genes) <= 1){
      next
    }
...
Table summarizing all the enriched pathways. The results can also be inspected on the browser query under this link: `r gostres_links`. This table can be also found under [results/pathway_analysis](./results/pathway_analysis/enriched_pathways_table.tsv).
