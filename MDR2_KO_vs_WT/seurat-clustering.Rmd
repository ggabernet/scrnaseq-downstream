...
subtitle: "MRD2 KO vs TGR5 high Mice liver cells"
...
```{r , echo=FALSE}
dataset_loc <- "./data_transgenics/"
ids <- c("216-01", "216-02", "216-03", 
         "216-04", "216-05", "216-09",
         "216-10", "216-11")

files <- c("sample-Lib-216-01/outs/filtered_feature_bc_matrix/", 
           "sample-Lib-216-02/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-03/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-04/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-05/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-09/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-10/outs/filtered_feature_bc_matrix/",
           "sample-Lib-216-11/outs/filtered_feature_bc_matrix/")

seurat_list <- list()
# Initialize the Seurat object with the raw (non-normalized data).
for (i in c(1:length(ids))) {
  seurat_data <- Read10X(data.dir = paste0(dataset_loc, files[i]))
...
```{r, echo=FALSE}
dkfz.nofilter <- merge(seurat_list[[1]], y = c(seurat_list[[2]],
                                          seurat_list[[3]],
                                          seurat_list[[4]],
                                          seurat_list[[5]],
                                          seurat_list[[6]],
                                          seurat_list[[7]],
                                          seurat_list[[8]]), 
                  add.cell.ids = c("C01","C02","C03","C04","C05","C09","C10","C11"), 
                  project = "DKFZ_TGR5")
```


```{r, out.width="50%", echo=FALSE}
tab <- as.data.frame(table(dkfz.nofilter$orig.ident))
...
```{r, echo=FALSE}
samplename = dkfz.nofilter@meta.data$orig.ident

batchid = rep("TGR5_high",length(samplename))
batchid[samplename %in% c("216-01", "216-03", "216-04", "216-05")] = "MDR2_KO"
names(batchid) = rownames(dkfz.nofilter@meta.data)

dkfz.nofilter <- AddMetaData(
  object = dkfz.nofilter,
  metadata = batchid,
  col.name = "grouping")
```

```{r, out.width="50%", echo=FALSE}
tab <- as.data.frame(table(dkfz.nofilter$grouping))
colnames(tab) <- c("Condition", "Cell_number")
kable(tab) %>%
    kable_styling("hover")
```
...
```{r, echo=FALSE}
dkfz.nofilter$percent.mt <- PercentageFeatureSet(dkfz.nofilter, pattern = "^mt-", assay = 'RNA')
#kable(head(dkfz.nofilter@meta.data, 5))
```
...
metadata <- dkfz.nofilter@meta.data
...
corr <- FeatureScatter(object = dkfz.nofilter, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
corr
...
```{r, out.width="90%", fig.align="center", echo=FALSE}
### GenePlot(object = seurat, gene1 = ?, gene2 = ?)
dkfz.nofilter@meta.data$gene_count_rank <- rank(1/dkfz.nofilter@meta.data$nFeature_RNA)
ggplot(data = dkfz.nofilter@meta.data, aes(y=nFeature_RNA, x=gene_count_rank)) +
  geom_point() + 
...
```{r, echo=FALSE}
#dkfz.all <- subset(dkfz.nofilter, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)
dkfz.all <- subset(dkfz.nofilter, subset = percent.mt < 10 & nFeature_RNA > 200 & nFeature_RNA < 5500)
...
qc_vln <- vln1 + vln2 + vln3
qc_vln
ggsave(paste0(results_folder,"quality_violin_plot_after_filtering.png"), qc_vln, device="png", width = 25, height = 8, units = "cm")
ggsave(paste0(results_folder,"quality_violin_plot_after_filtering.pdf"), qc_vln, device="pdf", width = 25, height = 8, units = "cm")
```
...
macro_markers <- FeaturePlot(dkfz.all, 
                         features = c("Adgre1", "Itgam", "Cd68", "Lyz2", "C1qb", "Clec4f"),
                         ncol = 3)
macro_markers
...
kupffer_markers <- FeaturePlot(dkfz.all, 
                           features = c("Adgre1", "Clecsf", "Clec4f", "Cd68", "Marco", "Vsig4", "Vcam1" ))
kupffer_markers
ggsave(paste0(results_folder,"markers/kupffer_markers_featureplot.pdf"), kupffer_markers, device="pdf", width = 20, height = 15, units = "cm")
ggsave(paste0(results_folder,"markers/kupffer_markers_featureplot.png"), kupffer_markers, device="png", width = 20, height = 15, units = "cm")
```
...
stellate_markers <- FeaturePlot(dkfz.all, 
                            features = c("Des", "Reln", "Dcn", "Acta2", "Col1a1", 
                                         "Col1a2", "Sparc", "Tagln"),
                            ncol = 3)
stellate_markers
ggsave(paste0(results_folder,"markers/stellate_markers_featureplot.pdf"), stellate_markers, device="pdf", width = 25, height = 25, units = "cm")
ggsave(paste0(results_folder,"markers/stellate_markers_featureplot.png"), stellate_markers, device="png", width = 25, height = 25, units = "cm")
```
...
new.cluster.ids <- c("Cholangiocytes", "Macrophage / Kupffer", "Hepatocytes", "Hepatocytes", "HSC", "Endothelial", "Macrophage / Kupffer", "Endothelial", "T-lymphocytes", "Endothelial vascular", "Macrophage / Kupffer", "Fibroblasts", "Cholangiocytes", "HSC", "B-lymphocytes", "Macrophage / Kupffer", "Lymphocytes", "Hepatocytes")
...
write.table(top10, file = paste0(results_folder,"top_10_markers_per_cluster_with_cellnames.tsv"), sep = "\t", quote = F, row.names = F)
...
KO.response <- FindMarkers(dkfz.all, ident.1 = "Cholangiocytes_TGR5_high", ident.2 = "Cholangiocytes_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Cholangiocytes_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp
ggsave(paste0(DE_folder,"volcano_plot_Cholangiocytes_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_Cholangiocytes_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
```{r echo=FALSE}
Idents(dkfz.all) <- "CellType"
cell <- subset(dkfz.all, idents = "HSC")
Idents(cell) <- "grouping"
avg.cell <- log1p(AverageExpression(cell, verbose = FALSE)$RNA)
avg.cell$gene <- rownames(avg.cell)

Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "HSC_TGR5_high", ident.2 = "HSC_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_HSC_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_HSC_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_HSC_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
```{r echo=FALSE}
Idents(dkfz.all) <- "CellType"
cell <- subset(dkfz.all, idents = "Fibroblasts")
Idents(cell) <- "grouping"
avg.cell <- log1p(AverageExpression(cell, verbose = FALSE)$RNA)
avg.cell$gene <- rownames(avg.cell)

Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "Fibroblasts_TGR5_high", ident.2 = "Fibroblasts_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Fibroblasts_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_Fibroblasts_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_Fibroblasts_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "Macrophage / Kupffer_TGR5_high", ident.2 = "Macrophage / Kupffer_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Macrophage_Kupffer_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Macrophage_Kupffer_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Macrophage_Kupffer_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
```{r echo=FALSE}
Idents(dkfz.all) <- "CellType"
cell <- subset(dkfz.all, idents = "Endothelial")
Idents(cell) <- "grouping"
avg.cell <- log1p(AverageExpression(cell, verbose = FALSE)$RNA)
avg.cell$gene <- rownames(avg.cell)

Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "Endothelial_TGR5_high", ident.2 = "Endothelial_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Endothelial_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Endothelial_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Endothelial_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
```{r echo=FALSE}
Idents(dkfz.all) <- "CellType"
cell <- subset(dkfz.all, idents = "Endothelial vascular")
Idents(cell) <- "grouping"
avg.cell <- log1p(AverageExpression(cell, verbose = FALSE)$RNA)
avg.cell$gene <- rownames(avg.cell)

Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "Endothelial vascular_TGR5_high", ident.2 = "Endothelial vascular_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Endothelial_vascular_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Endothelial_vascular_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Endothelial_vascular_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```
...
Idents(dkfz.all) <- "celltype.stim"
KO.response <- FindMarkers(dkfz.all, ident.1 = "Hepatocytes_TGR5_high", ident.2 = "Hepatocytes_MDR2_KO", verbose = FALSE)
KO.response$gene <- row.names(KO.response)
KO.response$log_pval_adj <- -log10(KO.response$p_val_adj)
kable(head(KO.response, n = 20)) %>%
    kable_styling("hover")
write.table(KO.response[,c("gene","avg_logFC","p_val_adj")], file = paste0(DE_folder, "DE_genes_Hepatocytes_TGR5_vs_MDR2KO.tsv"), sep = "\t", quote = F, row.names = F)
```
...
vp <- LabelPoints(plot = vp, points = genes.to.label, repel = TRUE)
vp
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Hepatocytes_TGR5_vs_MDR2KO.png"), vp, device="png", width = 10, height = 8, units = "cm")
ggsave(paste0(DE_folder,"volcano_plot_DE_genes_Hepatocytes_TGR5_vs_MDR2KO.pdf"), vp, device="pdf", width = 10, height = 8, units = "cm")
```

Saving R Seurat object:

```{r echo=FALSE}
saveRDS(dkfz.all, file = paste0(results_folder,"R_object_DKFZ_TGR5_vs_MDR2KO_cluster_assignments.rds"))
```
